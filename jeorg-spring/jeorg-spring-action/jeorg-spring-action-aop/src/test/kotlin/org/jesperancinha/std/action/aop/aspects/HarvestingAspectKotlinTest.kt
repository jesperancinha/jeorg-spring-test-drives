...
internal class HarvestingAspectKotlinTest @Autowired constructor(
    ...
    fun testAnyHarvesterWhenCall3HarversterTypesNSubsThenTriggerAdvices() {
        ...
        joinPointArgumentSlot.map { joinPoint: JoinPoint -> joinPoint.signature.toString() }
            ...
            .shouldHaveSize(2)
            .shouldContainAll(
                "Shrimp org.jesperancinha.std.action.aop.fishing.Shrimper.harvest()",
                "SeaFood org.jesperancinha.std.action.aop.fishing.Fisher.harvest()"
            )
        verify { harvestingService.targetShrimper(capture(joinPointArgumentSlot.also { it.clear() })) }
        joinPointArgumentSlot.first().signature.toString() shouldBe "Shrimp org.jesperancinha.std.action.aop.fishing.Shrimper.harvest()"
        ...
        joinPointArgumentSlot.map { joinPoint: JoinPoint -> joinPoint.signature.toString() }
            .shouldNotBeNull()
            .shouldHaveSize(1)
            .shouldContainAll("Shrimp org.jesperancinha.std.action.aop.fishing.Shrimper.harvest()")
    }
    ...
    fun testFishAnythingWhenCallReferenceToFisherThenTriggerOnlyFisherRefAdvicesAndAllInstanceShrimperAdvices() {
        ...
        joinPointArgumentSlot.map { joinPoint: JoinPoint -> joinPoint.signature.toString() }
            .shouldNotBeNull()
            .shouldHaveSize(1)
            .shouldContainAll("void org.jesperancinha.std.action.aop.fishing.Fisher.justFishAnything()")
        verify { harvestingService.targetShrimper(capture(joinPointArgumentSlot.also { it.clear() })) }
        joinPointArgumentSlot.first().signature.toString() shouldBe "void org.jesperancinha.std.action.aop.fishing.Fisher.justFishAnything()"
        verify { harvestingService.targetFisher(capture(joinPointArgumentSlot.also { it.clear() })) }
    ...
    fun testFishShrimpWhenCallParameterThenTriggerOnlyFisherRefAdvicesAndAllInstanceShrimperAdvices() {
        ...
        joinPointArgumentSlot.first().signature.toString() shouldBe "void org.jesperancinha.std.action.aop.fishing.Shrimper.secretHarvest()"
        ...
        joinPointArgumentSlot.map { joinPoint: JoinPoint -> joinPoint.signature.toString() }
            .shouldNotBeNull()
            .shouldHaveSize(1)
            .shouldContainAll("void org.jesperancinha.std.action.aop.fishing.Shrimper.secretHarvest()")
        verify { harvestingService.targetHarvester(capture(joinPointArgumentSlot.also { it.clear() })) }
        joinPointArgumentSlot.map { joinPoint: JoinPoint -> joinPoint.signature.toString() }
            .shouldNotBeNull()
            .shouldHaveSize(1)
    ...
    fun testFishShrimpWhenCallParameterThenProxyServiceGetsIntoAdvice() {
        ...
        joinPointArgumentSlot
            .shouldNotBeNull()
            .first().signature.toString() shouldBe "void org.jesperancinha.std.action.aop.fishing.Shrimper.secretHarvest()"
        shrimperArgumentSlot.captured.shouldNotBeNull()
            .apply {
