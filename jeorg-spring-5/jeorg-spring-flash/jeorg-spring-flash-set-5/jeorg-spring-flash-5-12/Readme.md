...
```bash
mvn clean install spring-boot:run -Dspring-boot.run.profiles=command
```

1. Run the service with the SpringFlash512ApplicationLauncher

```bash
mvn clean install spring-boot:run -Dspring-boot.run.profiles=application
```
